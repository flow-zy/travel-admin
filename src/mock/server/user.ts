...
for (let i = 0; i < 10; i++) {
  userList.push({
    ...
    role: `document,users,${Random.pick(permissionArr.filter(i => !['document', 'users'].includes((i.name as string))))}`,
    avatar:`https://picsum.photos/id/${i *Math.round(Math.random()*(99 - i)) +1}/200/200`
  })
}
// 登录
